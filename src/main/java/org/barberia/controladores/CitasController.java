...
public String guardar(@ModelAttribute("cita") Citas cita,
    ...
    cita.setHoraFin(horaInicio != null ? horaInicio.plusHours(1) : null);
    ...
    try {
        citasService.validarYGuardarCita(cita);
    } catch (IllegalArgumentException e) {
        logger.warn("No se pudo guardar la cita: {}", e.getMessage());
        ...
        if (!esAdmin && !esBarbero) {
            ...
            model.addAttribute("clientes", usuariosRepository.findAll());
        }

        model.addAttribute("servicios", serviciosRepository.findAll());
        model.addAttribute("barberos", barberosRepository.findAll());
        model.addAttribute("esNuevo", cita.getId() == null);

        return "citas/formulario";
        }

        // Notificaciones por correo si es nueva cita
